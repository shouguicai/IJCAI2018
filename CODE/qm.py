...
if __name__ == "__main__":
    ...
    data = pd.read_csv('data/origin/round1_ijcai_18_train_20180301.csv')
    ...
    if online == False:
        ...
        train = data.copy()
        test = pd.read_csv('data/origin/round1_ijcai_18_test_a_20180301.csv')
    ...
    if online == False:
        clf = lgb.LGBMClassifier(num_leaves=63, max_depth=7, n_estimators=200, n_jobs=20)
        clf.fit(train[features], train[target],feature_name=features,eval_set=[(train[features], train[target]),(test[features], test[target])],
        eval_metric='binary_logloss',categorical_feature=['user_gender_id', ],early_stopping_rounds=50)
        ...
        output.to_csv('result/submission_qm_3_25.txt', index=False, sep=' ', columns={'instance_id', 'predicted_score'})
